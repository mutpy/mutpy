...
#poważna zmiana w kodzie
def copy_node(mutate):
    def f(self, node):
        type_mark_dict = {}
        id_number = 0
        type_mark = annotations.mark_name
        type_mark_id = 'type_mark_id'
        strip_type_mark(node,type_mark_dict,type_mark,type_mark_id)
        Memo = {id(node.parent): node.parent,}
        copied_node = copy.deepcopy(node, memo=Memo)
        reapply_type_mark(node,type_mark_dict,type_mark,type_mark_id)
        reapply_type_mark(copied_node,type_mark_dict,type_mark,type_mark_id)
        
        return mutate(self, copied_node)
    
    def strip_type_mark(node,type_mark_dict,type_mark = 'type_mark',type_mark_id = 'type_mark_id'):
        id_number = 0
        if hasattr(node,type_mark):
            type_mark_dict[id_number] = getattr(node,type_mark)
            setattr(node,type_mark_id,id_number)
            delattr(node,type_mark)
            id_number+=1
        for child in node.children:
            if hasattr(child,type_mark):
                type_mark_dict[id_number] = getattr(child,type_mark)
                setattr(child,type_mark_id,id_number)
                delattr(child,type_mark)
                id_number+=1     	
    def reapply_type_mark(node,type_mark_dict,type_mark = 'type_mark',type_mark_id = 'type_mark_id'):
        if hasattr(node,type_mark_id):
            id_number = getattr(node,type_mark_id)
            attr_to_set = type_mark_dict[id_number]
            setattr(node,type_mark,attr_to_set)
            delattr(node,type_mark_id)
        for child in node.children:
            if hasattr(child,type_mark_id):         
                setattr(child,type_mark,type_mark_dict[getattr(child,type_mark_id)])
                delattr(child,type_mark_id)   	
    return f


class MutationOperator:

    def mutate(self, node, to_mutate=None, sampler=None, coverage_injector=None, module=None, only_mutation=None, type_check = False):
		...
		#Zmiana w kodzie
        self.annotation_manager = annotations.get_annotation_manager(type_check)
        annotations.controlledPrint('\n-------------\nnew mutate with {}\n-------------'.format(self.__class__))
        
        for new_node in self.visit(node):
            annotations.controlledPrint('Node from self.visit: {}'.format(new_node))
            yield Mutation(operator=self.__class__, node=self.current_node, visitor=self.visitor), new_node

    def visit(self, node):
        annotations.controlledPrint('\n    IN VISIT   node:{0}'.format(node.__str__()),end="")
        if self.has_notmutate(node) or (self.coverage_injector and not self.coverage_injector.is_covered(node)):
            annotations.controlledPrint(" killed by has_notmutate {0} or coverage_injector {1}".format(self.has_notmutate(node),(self.coverage_injector and not self.coverage_injector.is_covered(node))),end="")
            return
        if self.only_mutation and self.only_mutation.node != node and self.only_mutation.node not in node.children:
            annotations.controlledPrint(" killed by only_mutation",end="")
            return
        self.fix_lineno(node)
        visitors = self.find_visitors(node)
        annotations.controlledPrint('visitors {}'.format(len(visitors)))
        if visitors:
            for visitor in visitors:
                try:
					...
					#Zmiana w kodzie
                    self.annotation_manager.visit_check_if_mutable(node,visitor)
                    ...
                    annotations.controlledPrint('node {0} mutated with {1}'.format(node,visitor))
                    yield new_node
                except MutationResign:
                    pass
                except annotations.MutationAnnResign:
                    pass
                finally:
    ...
    def generic_visit(self, node):
        annotations.controlledPrint('\nNODE:{0}, fields {1}\n{2}'.format(node,node._fields,node.__dict__),end="")
        for field, old_value in ast.iter_fields(node):
            annotations.controlledPrint("\nnode:{0} \tfield {1} old_value {2}".format(node,field,old_value),end="")
            if isinstance(old_value, list):
                annotations.controlledPrint(' 1',end="")
                generator = self.generic_visit_list(old_value)
            elif isinstance(old_value, ast.AST):
                annotations.controlledPrint(' 2',end="")
                generator = self.generic_visit_real_node(node, field, old_value)
            else:
                annotations.controlledPrint(' 3',end="")
                generator = []
    ...
    def generic_visit_list(self, old_value):
        ...
        for position, value in enumerate(old_values_copy):
            annotations.controlledPrint('\n\t in list position {0}, value {1}'.format(position,value),end="")
            if isinstance(value, ast.AST):
                for new_value in self.visit(value):
                    annotations.controlledPrint(" new_value, "+new_value.__str__(),end="")
...
class VariableReplacementWithConstants():
	pass
